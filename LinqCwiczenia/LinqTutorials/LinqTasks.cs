...
namespace LinqTutorials
{
    public static class LinqTasks
    {
        ...
        {
            return Emps.Any(emp => emp.Job == "Frontend programmer");
        }
        ...
        public static IEnumerable<object> Task10()
        {
            return Emps
                .Select(
                    emp => new { emp.Ename, emp.Job, emp.HireDate })
                .Union(
                    from emp in Emps 
        ...
        {
            return Emps.Join(Depts, emp => emp.Deptno, dept => dept.Deptno, (emp, dept) => new{dept.Dname, emp.Ename})
                .GroupBy(emp => emp.Dname)
                .Select(emp => new { name = emp.Key, numOfEmployees = emp.Count() }).ToList();
        }
        ...
        public static IEnumerable<object> Task15()
        {
            return (
                from emp in Emps
                where emp.Job.Contains("A") ||  emp.Job.Contains("a")
                group emp by emp.Job into a_jobs
                where a_jobs.Count() > 2
                orderby a_jobs.Count() descending
                select new { Job = a_jobs.Key, LiczbaPracownikow = a_jobs.Count() }
            );
        }
    ...
    public static class CustomExtensionMethods
    {
        ...
        {
            return emps
                .Where(emp => emps.Any(emp1 => emp1.Mgr == emp.Mgr))
